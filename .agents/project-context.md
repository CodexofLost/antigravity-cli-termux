...
- **Runtime Environment Isolation & Preload Bridging:**
  ...
  - Automatically builds, installs, and injects `libtermux-stat-fix.so` into `termux-shell` and `termux-shell-env`, intercepting `fstatat`/`fstat`/`stat`/`lstat`/`statx`/`access`/`faccessat` for `/storage/emulated` and `/storage/self` (including relative traversals, `AT_EMPTY_PATH`, and trailing slashes) with strict POSIX compliance and zero-lock constructor initialization to completely eliminate Git 2.35+ `fatal: detected dubious ownership` failures across Android shared storage (`/storage/emulated/0`).
